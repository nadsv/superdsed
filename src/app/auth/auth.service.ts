...
export class AuthService {
	...
	loginUser(name, pwd) {
		...
		const user = { 'name': name, 'password': this.MD5(pwd)};
    const data = JSON.stringify(user); 
    this.sedAPI.changeData(this.sedAPI.apiUrl + 'get-auth', data)
      .subscribe(
        (data) => { 
          if ( +data.result ) {
        ...
        () => { this.dialogsService.inform('Сообщение об ошибке', 'Нет доступа к базе данных пользователей.', false, this.vcRef); }
        )
